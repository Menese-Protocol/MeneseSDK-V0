# Build AI Agents with MeneseSDK

Turn any AI agent into a multi-chain crypto wallet. Your agent handles addresses, balances, sends, swaps, and bridges across 19 chains using MeneseSDK on the Internet Computer.

Works with ClawdBot (WhatsApp), LLM agents, custom bots — any agent that can run shell commands or call a canister.
...
Add the export line to your shell profile (`~/.bashrc`, `~/.zshrc`) so it persists.
...
### 3. Get your addresses

```bash
dfx canister call urs2a-ziaaa-aaaad-aembq-cai getAllAddresses '()' --network ic
```

Returns addresses for all 19 chains. Fund whichever chain you need by sending crypto to the corresponding address.
...
All address and balance queries are **free** — no subscription required.
...
```
User → AI Agent → Your Wallet Canister (ICP) → MeneseSDK canister
```

**Your canister** lives on ICP and makes inter-canister calls to MeneseSDK.
Your agent calls the canister via dfx or HTTP agent.
...
```
User → AI Agent → dfx canister call → MeneseSDK canister
```

Your agent runs `dfx canister call` commands directly from your VPS.
...
### 4. Configure your agent's system prompt

Add to your agent's context so it knows how to use the wallet:
...
Supported chains: sol, icp, btc, ltc, eth, arb, base, polygon, bsc, optimism, xrp, sui, ton, apt, near, trx, ada, cloak, rune
```
...
```bash
cp wallet_commands.py ~/workspace/
...
## Using SKILL.md with Your Agent

The `SKILL.md` file is the complete function catalog — 400+ endpoints across all 19 chains. Feed it directly to your AI agent as context:

```
# For any LLM agent (ClawdBot, custom bots, etc.):
# Include SKILL.md in the system prompt or tool context.
# The agent can then look up exact function signatures, parameters,
# and return types for any operation.
```

This is how ClawdBot and MeneseAgent work — they read SKILL.md at startup and know every available operation.
...
Free public RPCs work for testing. For production, use Alchemy, Infura,
or chain-specific premium RPCs for better reliability and throughput.

---

## Performance Tip: Direct Balance Queries

Balance queries through MeneseSDK go through the canister's shared endpoints.
For faster balance checks, query the chain directly with your own RPC:
...
Both approaches support natural language via your AI agent:
...
Address and balance queries are always free. All other operations cost 1 action.

| Tier | Monthly Price | Actions |
|------|--------------|---------|
| **Basic** | $20 | 100 |
| **Developer** | $45.50 | 1,000 |
| **Pro** | $128.70 | 5,000 |
| **Enterprise** | $323.70 | Unlimited |

```bash
# Check your current subscription status
dfx canister call urs2a-ziaaa-aaaad-aembq-cai getMyGatewayAccount '()' --network ic

# Purchase a subscription (pay with ICP or any supported token)
...
| File | Purpose |
|------|---------|
| `SKILL.md` | Complete function catalog (400+ endpoints) — feed to your AI agent |
| `WalletBot.mo` | Motoko canister wrapping MeneseSDK (Approach A) |
| `wallet_commands.py` | Python CLI for direct dfx calls (Approach B) |
| `references/api-surface.md` | Full API reference (367 function signatures) |
| `references/automation.md` | Automation patterns (DCA, stop-loss, rebalancing) |

## MeneseSDK Canister

- **Canister ID:** `urs2a-ziaaa-aaaad-aembq-cai`
- **Network:** Internet Computer mainnet
- **Supported chains (19):** Solana, Ethereum, Bitcoin, Arbitrum, Base, Polygon, BSC, Optimism, ICP, XRP, SUI, TON, Cardano, Aptos, NEAR, Tron, Litecoin, CloakCoin, THORChain
